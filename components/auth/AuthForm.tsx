...
function LeftPanel() {
  return (
    ...
    >
      ...
      >
        <h1 className="font-headline font-extrabold text-6xl tracking-tighter text-white mb-3">
          ChatVibe
        </h1>
        <p className="font-headline text-xl text-white/80 font-medium">
          Connect. Chat. Vibe.
        </p>
      </motion.div>
  ...
  return (
    <section className="w-full md:w-[55%] lg:w-1/2 bg-surface flex flex-col overflow-y-auto">
      ...
      <div className="md:hidden text-center pt-8 pb-4 px-6">
        <h1 className="font-headline font-extrabold text-4xl text-gradient">
          ChatVibe
        </h1>
    ...
    <main className="flex h-screen w-full overflow-hidden bg-surface">
      ...
      <RightPanel>
        <div className="mb-6">
          ...
          <p className="text-on-surface-variant text-sm mt-1">
            Sign in to stay connected with your vibe tribe.
          </p>
        </div>
        ...
        >
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              {errors.password && (
                <p className="text-error text-xs ml-1">
  ...
  const onSubmit = async (data: RegisterInput) => {
    ...
    try {
      ...
      if (result?.error) toast.error("Login failed after registration");
        ...
        toast.success("Welcome to ChatVibe! 🎉");
        router.push("/chat");
      }
    } catch (error: unknown) {
      const msg =
